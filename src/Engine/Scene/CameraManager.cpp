...
void CameraManager::Init(EngineCore::IApp* pApp)
{
    ...
    pApp->RegisterEventHandler(EngineCore::KEYBOARD, [this](EngineCore::IOInput input) {
        if (activeCamera->getType() == ECameraType::EUser && (input.action == GLFW_PRESS || input.action == GLFW_REPEAT)) {
            ...
            if (input.key == GLFW_KEY_W) {
                userCamera->fromPos += lookAtDir * speed;
                userCamera->lookAtPos += lookAtDir * speed;
                ...
                userCamera->fromPos -= right * speed;
                userCamera->lookAtPos -= right * speed;
            } else if (input.key == GLFW_KEY_D) {
                auto right = glm::cross(lookAtDir, upDir);
                right = glm::normalize(right);
                userCamera->fromPos += right * speed;
                userCamera->lookAtPos += right * speed;
            } else if (input.key == GLFW_KEY_Q) {
                userCamera->fromPos += upDir * speed;
                userCamera->lookAtPos += upDir * speed;
            } else if (input.key == GLFW_KEY_E) {
                userCamera->fromPos -= upDir * speed;
                userCamera->lookAtPos -= upDir * speed;
            } else if (input.key == GLFW_KEY_R) {
    ...
    pApp->RegisterEventHandler(EngineCore::MOUSE_MOVE, [this](EngineCore::IOInput input) {
        if (activeCamera->getType() == ECameraType::EUser && isMouseLeftButtonDown) {
            if (lastMousePos.x >= 0 && lastMousePos.y >= 0) {
                ...
                constexpr float sensitivity = glm::radians(1.0f);
                ...
                float horizAngle = sensitivity * mouseDelta.x * -1;
                float vertAngle = sensitivity * mouseDelta.y * -1;
                ...
                if (currentAngle > vertical_angle_limit_max || currentAngle < vertical_angle_limit_min) {
                    vertAngle = 0.0f; // This effectively cancels out the vertical rotation for this frame
                }

                // Apply vertical rotation after clamping
