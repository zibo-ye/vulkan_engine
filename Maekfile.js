...
function compileSource(file, basePath) {
    return maek.CPP(`${basePath}\\${file}`, undefined, {
        CPPFlags: [
            '-Isrc\\Engine',
            '-DGLFW_INCLUDE_NONE',
            `-I${VULKAN_SDK}/Include`,
...
async function compileEngineSources(dir) {
    let files = await fsPromises.readdir(dir, { withFileTypes: true });
    let cppFiles = files.filter(file => file.isFile() && file.name.endsWith('.cpp'));
    let dirs = files.filter(file => file.isDirectory());

    let sources = [];
    
    // Compile all .cpp files in the current directory
    for (let file of cppFiles) {
        sources.push(compileSource(file.name, dir));
    }

    // Recursively compile .cpp files in subdirectories
    for (let d of dirs) {
        let subDirSources = await compileEngineSources(path.join(dir, d.name));
        sources.push(...subDirSources);
    }
...
async function main() {
    ...
    await maek.update(maek.TARGETS);
}
